import NameNumberCard from "./NameNumberCard";
import OnlineOflineButtons from "./OnlineOflineButtons";


export default function RightBusinessCard() {
 

  return (
    <div className="w-70 2xl:w-xs bg-[#FFF8F3] px-3 py-4 rounded-xl shadow-lg ">
      <h2 className="text-sm font-bold text-gray-800 mb-1">
        Explore the Top <span className="text-[#FF6E04]">Coaching Centers</span>
      </h2>
      <p className="text-[11px] font-[500] tracking-tight mb-4">
        You will receive the contact details instantly, at no cost
      </p>

      <p className="text-sm font-[500] pr-3 mb-2">
        Tell us the type of Coaching Institutes you need?
      </p>
 
     <div className="mt-4 mb-10 px-3">

       <OnlineOflineButtons />
     </div>
    

     <NameNumberCard layout={"col"} />

      <button className="w-full bg-[#FF6E04] cursor-pointer text-sm text-white font-semibold py-2 rounded-lg hover:bg-orange-600 transition">
        UNLOCK TOP DEALS
      </button>
    </div>
  );
}
