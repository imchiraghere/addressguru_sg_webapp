...
const ReviewSlider = () => {
  ...
  return (
    <div className="relative w-full py-20  overflowx-visible">
      {/* Arrow buttons */}
      {/* <button
        onClick={prevSlide}
        className="absolute left-5 top-1/2 transform -translate-y-1/2 z-30 bg-gray-200 p-2 rounded-full"
      >
        ←
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-5 top-1/2 transform -translate-y-1/2 z-30 bg-gray-200 p-2 rounded-full"
      >
        →
      </button> */}

      <div className="relative max-w-md flex max-h-50 justify-center right-110 2xl:w-lg  top-20">
          ...
          return (
            <div
              ...
              className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 
                ...
                rounded-xl p-6 shadow-lg transition-all duration-500 ease-in-out text-center flex flex-col items-center w-[300px]`}
            >
      ...
      <div className="flex justify-center mt-32 gap-3">
        {REGISTER_REVIEWS.slice(0, 3).map((_, index) => (
          <span
            key={index}
            className={`h-2 w-2 relative right-74 top-30 rounded-full cursor-pointer ${
